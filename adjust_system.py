...
Usage:
    python ADJUST_SYSTEM.py <pdb_file> <xml_file> [--overwrite]
...
With --overwrite, no new files are created; the force-group + HMR adjusted
system replaces <xml_file> in place.
"""
...
def adjust_system(pdb_file: str, xml_file: str, overwrite: bool = False):
    """
    ...
    overwrite : bool
        If True, write the adjusted system back to xml_file instead of
        creating new _FG_sys.xml / _FG_HMR_sys.xml files.
    ...
    If overwrite is True, both stages are written to xml_file, leaving the
    force-group + HMR adjusted system in place of the original.
    """
    # Validation
    ...
    # Write force-group adjusted system (skipped when overwriting, since the
    # HMR write below targets the same file)
    if not overwrite:
    ...
    mass_before = np.array([sys.getParticleMass(i) for i in range(sys.getNumParticles())]).sum()
    ...
    mass_after = np.array([sys.getParticleMass(i) for i in range(sys.getNumParticles())]).sum()
    mass_delta = mass_after - mass_before

    #print(f"  Scaled {len(scale_plan)} hydrogen bonds")
    print(f"  Mass before: {mass_before._value:.6f}")
    print(f"  Mass after:  {mass_after._value:.6f}")
    print(f"  Delta:       {mass_delta._value:.9f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--overwrite',
        action='store_true',
        help='Overwrite the input XML file instead of writing new _FG/_FG_HMR files',
    )

    args = parser.parse_args()

    adjust_system(args.pdb_file, args.xml_file, overwrite=args.overwrite)
